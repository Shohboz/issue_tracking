...
import API from "redux/api/departments";
    ...
    return API.create(data)
      ...
      .then(response => {
        if (response.ok) {
          ...
          createNotification(dispatch, {
            type: FAIL,
            text: "Создание отдела завершилось ошибкой"
          });
        }
    ...
    return API.update(data)
      ...
      .then(response => {
        if (response.ok) {
          createNotification(dispatch, {
            type: SUCCESS,
            text: `Отдел #${response.json.id} успешно обновлен`
          });
          dispatch({ type: UPDATE_OK, payload: response.json });
        } else {
          createNotification(dispatch, {
            type: FAIL,
            text: "Обновление отдела завершилось ошибкой"
          });
        }
